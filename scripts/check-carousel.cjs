...
const context = {window, document, matchMedia:()=>desktop, getComputedStyle:()=>({columnGap:'18px'}),
  ...
  IntersectionObserver:class {constructor(fn){this.fn=fn;}observe(){this.fn([{isIntersecting:true,intersectionRatio:1}]);}}
};
...
const api=window.createDesktopReelCarousel({track,previous,next,reduced,saveData:false,onChange(){},isModalOpen:()=>false});
advance(200);
...
console.log('PASS: 48 circular moves, both boundaries, fractional pixel positions, hover, drag, automatic advance, mobile drag and height-only resize.');
