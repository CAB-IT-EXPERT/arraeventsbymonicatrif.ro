...
(async () => {
  ...
  const publicFiles = manifest.filter(item => !item.file.endsWith('.htaccess') && !item.file.endsWith('.php'));
  ...
  const configurator = await fetch(origin + 'cabitexpert/alege-urmatoarele-servicii/');
  const configuratorHtml = await configurator.text();
  assert.equal(configurator.status, 200);
  assert(configuratorHtml.includes('name="robots" content="noindex, nofollow'));
  assert((configurator.headers.get('x-robots-tag') || '').includes('noindex'));
  const configuratorApi = await fetch(origin + 'cabitexpert/alege-urmatoarele-servicii/api.php');
  const configuratorState = await configuratorApi.json();
  assert.equal(configuratorApi.status, 200);
  assert.equal(configuratorState.ok, true);
  ...
  const result = {verifiedAt:new Date().toISOString(),origin,filesWithMatchingSHA256:checked,homepage:200,httpRedirect:301,privateHtaccess:403,custom404:404,videoRange:206,openGraphLogo:true,privateConfigurator:200,configuratorApi:200};
